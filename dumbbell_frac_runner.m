...
rho = 0.05 : 0.05 : 0.95; % Traffic load at the bottleneck node
k = 5; % Number of links
...
policy = 1; % 0 for FCFS, 1 for OPF, 2 for HAF
...
for ki = 1 : length(zetas)
    ...
    for j = 1 : length(rho)
        ...
        lambda(1) = lambda(1) * N * zeta;
        lambda(2) = lambda(2) * N * (1 - zeta);
