function [departure_times] = multi_source_finite_queue(arrival_times, origin_times, mu, epsilon, policy, queue)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
arrivals = ones(1, num_packets * k);
departures = ones(1, num_packets * k);
...
if (policy > 0)
    % In OPF/HAF, priority is different, but the first packet to arrive is
...
while (min(next_index) <= num_packets)
    ...
    for i = 1 : length(next_source)
        queued = queued + length(find(departure_times(next_source, max(1, next_index(next_source) - 1 - queue * 2) : max(1, next_index(next_source) - 1)) > arrival_times(next_source, next_index(next_source))));
    end
    % Check if the queue is full
    if (queued == queue)
        arrival_times(next_source, next_index(next_source)) = -1;
            ...
            if (next_index(source) > num_packets)
                ...
                if (empty)
                    ...
                    if (policy == 1)
                        ...
                        % In HAF, priority is the AoI (ie, origin time of
                        % the previous packet)
